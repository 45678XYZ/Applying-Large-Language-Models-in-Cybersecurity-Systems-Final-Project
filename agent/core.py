...
class SecurityAgent:
    ...
    def _synthesis_context(
        ...
        return (
            ...
            "not tied to a specific scanned port. ALSO, when the CVE context "
            ...
            "iot_exposure for another device). Mark it high only when the device's "
            "scanned version confirms it is in the CVE's affected range; if only the "
            "product family matches or the version is unknown, treat it as a possible "
            "match (medium, or low when CVSS < 7.0) and say the firmware version "
